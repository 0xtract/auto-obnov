    ...
    async def roles_delete(guild_to: discord.Guild):
            for role in guild_to.roles:
                try:
                    ...
                    print_error(f"Ошибка при удаление роли: {role.name}")
                except discord.HTTPException:
                    print_error(f"Не удалось удалить роль: {role.name}")
    ...
    async def roles_create(guild_to: discord.Guild, guild_from: discord.Guild):
        ...
        for role in roles:
            try:
                ...
                print_error(f"Ошибка при создание роли: {role.name}")
            except discord.HTTPException:
                print_error(f"Не удалось создать роль: {role.name}")
    ...
    async def channels_delete(guild_to: discord.Guild):
        for channel in guild_to.channels:
            try:
                ...
                print_error(f"Ошибка при удаление канала: {channel.name}")
            except discord.HTTPException:
                print_error(f"Не удалось удалить канал: {channel.name}")
    ...
    async def categories_create(guild_to: discord.Guild, guild_from: discord.Guild):
        ...
        for channel in channels:
            try:
                ...
                print_error(f"Ошибка при создании категории: {channel.name}")
            except discord.HTTPException:
                print_error(f"Не удалось создать категорию: {channel.name}")
            ...
            try:
                for category in guild_to.categories:
                    try:
                        ...
                        print_warning(f"Текстовый канал {channel_text.name} находиться без категории!")
                        category = None
                        break
                ...
                print_error(f"Ошибка при создании текстового канала: {channel_text.name}")
            except discord.HTTPException:
                print_error(f"Не удалось создать текстовый канал: {channel_text.name}")
            except:
                print_error(f"Ошибка при создании текстового канала: {channel_text.name}")
            ...
            try:
                for category in guild_to.categories:
                    try:
                        ...
                        print_warning(f"Голосовой канал {channel_voice.name} находиться без категории!")
                        category = None
                        break
                ...
                print_error(f"Ошибка при создании голосового канала: {channel_voice.name}")
            except discord.HTTPException:
                print_error(f"Не удалось создать голосовой канал: {channel_voice.name}")
            except:
                print_error(f"Ошибка при создании голосового канала: {channel_voice.name}")
    ...
    async def emojis_delete(guild_to: discord.Guild):
        for emoji in guild_to.emojis:
            try:
                ...
                print_error(f"Ошибка при удалении эмодзи{emoji.name}")
            except discord.HTTPException:
                print_error(f"Ошибка при удалении эмодзи {emoji.name}")
    ...
    async def emojis_create(guild_to: discord.Guild, guild_from: discord.Guild):
        ...
        for emoji in guild_from.emojis:
            try:
                ...
                print_error(f"Ошибка при создании эмодзи {emoji.name} ")
            except discord.HTTPException:
                print_error(f"Ошибка при создании эмодзи {emoji.name}")
